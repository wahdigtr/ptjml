 ...
 */
	public $settings = array(
		'table' => array(
			'class' => 'dataTable table table-striped table-condensed table-bordered table-hover',
			//'id' => 'data',
			'trOptions' => array(),
			'thOptions' => array(),
			'theadOptions' => array(),
		...
		'js' => array(
			...
			//'columnDefs' => array(array("targets" => array(2), "visible" => false))
			//'bLengthChange' => false '[{"target":[0], "visible":false}]'
 		),
	);
...
$(document).ready(function() {
		...
		var mytable = table.dataTable(settings);
		$('.dataTables_filter label').addClass('full-width');
		$('.DTTT_container').addClass('no-padding');

		var inputGroup =  '<div class="input-group input-medium">'+
		                  '<input type="text" class="form-control" placeholder="Search ">'+
		                  '</div>';
		$('.dataTables_filter label').html(inputGroup);
		...
		$('.dataTables_filter input')
		    ...
		    $('#dataTables_filter__button')
		    .bind('click', function(e){
		      if(text != $('.dataTables_filter input').val() ){
		        hit = 0;
		        if(hit == 0){
		          mytable.fnFilter($('.dataTables_filter input').val());
		          hit = 1;
		        }
		        text = $(this).val();
		      }else{
